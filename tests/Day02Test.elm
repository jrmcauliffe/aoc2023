...
suite =
    describe "Day2 Tests"
            ...
            [ test "Given Example" <|
                \_ ->
                    example
                        |> parse
                        |> List.map parseGame
                        |> List.filterMap (gamePossible 12 13 14)
                        |> List.map (\g -> g.num)
                        |> List.sum
                        |> equal 8
            , test "Problem" <|
        ...
        , describe "Part2"
            [ test "Given Example" <|
                \_ -> example |> parse |> List.map parseGame |> List.map minGamePower |> List.sum |> equal 2286
            , test "Problem" <|
                \_ -> problem |> parse |> List.map parseGame |> List.map minGamePower |> List.sum |> equal 67953
            ]
        ]
